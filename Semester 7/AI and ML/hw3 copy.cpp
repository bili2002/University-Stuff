...
private:
    ...
    // Weight random subset by fitness and then choose 2 random individuals using the weights
    pair<Individual, Individual> selection() const { 
        vector<int> perm = generate_random_permutation(population.size());
        vector<Individual> subset;
        for (int i = 0; i < SELECTION_SIZE; ++i) {
            subset.push_back(population[perm[i]]);
        }

        double total_fitness = 0.0;
        for (const auto& ind : subset) {
            total_fitness += ind.fitness;
        }

        vector<double> fitness_probabilities(SELECTION_SIZE);
        for (size_t i = 0; i < subset.size(); ++i) {
            fitness_probabilities[i] = subset[i].fitness / total_fitness;
        }

        std::discrete_distribution<> dis(fitness_probabilities.begin(), fitness_probabilities.end());

        int parent1_idx = dis(gen);
        int parent2_idx;
        do {
            parent2_idx = dis(gen);
        }
        ...
        return {subset[parent1_idx], subset[parent2_idx]};
    }
    ...
    std::vector<int> generate_random_permutation(int n) const {
...
int main() {
    ...
    GeneticAlgorithm ga(1000, 100, 5, 0.7, 0.5, 0.01);
